...
export default function InvoicePrint({ order, settings, onClose }: InvoicePrintProps) {
  const [isDownloading, setIsDownloading] = React.useState(false);
  
  const balanceDue = order.balancePayment;
  ...
  // upi://pay?pa=7905256355@upi&pn=Shiv%20Studio&am=20000&cu=INR
  ...
  const handleWhatsAppShare = () => {
    const formattedMobile = order.customerMobile.length === 10 ? `91${order.customerMobile}` : order.customerMobile;
    const message = `*SHIV STUDIO & PRINTERS, KHAGA*\n\nDear *${order.customerName}*,\n\nHere are details of your order *${order.orderId}*:\n• *Service:* ${order.serviceType}\n• *Booking Date:* ${order.orderDate}\n• *Delivery Target:* ${order.deliveryDate}\n• *Total Amount:* ₹${order.totalAmount.toLocaleString('en-IN')}\n• *Advance Paid:* ₹${order.advancePayment.toLocaleString('en-IN')}\n• *Balance Due:* ₹${order.balancePayment.toLocaleString('en-IN')}\n• *Work Stage:* ${order.status}\n\n*Payment Details:*\nScan the QR code on your printed receipt or pay via UPI on *${settings.upiId}*.\n\nThank you for choosing Shiv Studio & Printers!\n📞 Contact: ${settings.mobile}`;
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const element = document.getElementById('print-receipt-a4');
      if (!element) {
  ...
  return (
    <div id="invoice-overlay" className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-2xl transition-colors max-w-3xl mx-auto">
      
      {/* 1. Control Action Bar (Hidden during window.print) */}
      <div id="invoice-control-bar" className="bg-slate-100 dark:bg-slate-800 px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex flex-wrap items-center justify-between gap-3 print:hidden transition-colors">
        <div className="flex items-center gap-2">
          <Receipt className="h-5 w-5 text-amber-500" />
          <span className="font-extrabold text-sm text-slate-800 dark:text-white uppercase tracking-wider">A4 Printable Receipt</span>
        </div>
      ...
      <div id="print-receipt-a4" className="print-invoice-target p-8 bg-white text-slate-800 print:p-0 print:text-black">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 pb-6 border-b-2 border-slate-200">
          ...
          <div className="text-left md:text-right space-y-2 shrink-0 md:w-56">
            ...
            <div className="text-xs font-semibold text-slate-600 space-y-1">
              <p><span className="text-slate-400">Invoice No:</span> <strong className="text-slate-800 font-mono">{order.orderId}</strong></p>
              <p><span className="text-slate-400">Order Date:</span> <strong>{new Date(order.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</strong></p>
              <p><span className="text-slate-400">Delivery Target:</span> <strong className="text-slate-800">{new Date(order.deliveryDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</strong></p>
            </div>
          </div>
        ...
        <div className="py-6 border-b border-slate-100 flex flex-col sm:flex-row justify-between gap-4 text-xs font-semibold">
          <div className="space-y-1">
            ...
            <p className="text-sm font-black text-slate-800 leading-tight">{order.customerName}</p>
            <p className="text-slate-500 font-bold">Phone: {order.customerMobile}</p>
            {order.customerAddress && (
              <p className="text-slate-400 max-w-sm font-medium">{order.customerAddress}</p>
            )}
          </div>
          ...
          <div className="sm:text-right space-y-1">
            ...
            <span className="inline-block bg-slate-100 text-slate-800 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider">
              {order.category === 'studio' ? '📸 Studio Photography' : '🖨️ Printing Press Job'}
            </span>
            <p className="text-slate-400 text-[11px] mt-1">Status: <strong className="text-slate-700 uppercase">{order.status}</strong></p>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse text-xs font-semibold">
            <thead>
              <tr className="border-b-2 border-slate-200 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                ...
                <th className="py-2.5">Service Item &amp; Specifications</th>
            ...
            <tbody className="text-slate-700">
              <tr className="border-b border-slate-100">
                ...
                <td className="py-4 space-y-1">
                  ...
                  <p className="text-[11px] text-slate-400 font-medium leading-relaxed max-w-lg">{parsedItem.description}</p>
                </td>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-6 border-t border-slate-200">
          ...
          <div className="space-y-2.5 text-xs font-semibold text-slate-600">
            ...
            <div className="flex justify-between border-b border-slate-100 pb-1.5">
              ...
              <span className="text-slate-800">₹{order.totalAmount.toLocaleString('en-IN')}</span>
            </div>
            ...
            <div className="flex justify-between border-b border-slate-100 pb-1.5">
              ...
              <span className="text-emerald-600 font-bold">₹{order.advancePayment.toLocaleString('en-IN')}</span>
            </div>
